...
팩션 AI는 백그라운드에서 수치를 계산하는 '내부 연산'과, 계산된 결과에 따라 실제 개체의 행동을 제어하는 '상태 머신(FSM) / 비헤이비어 트리'로 분리되어 작동합니다.

| 내부 연산 (Internal Calculation) | 상태 머신 및 행동 제어 (FSM / Behavior Tree) |
| --- | --- |
| **우호도 연산** (Hostility Calculation) | **Friendly (우호)** : 버프 제공 / 아군 참전 |
| **알파 사망 연산** (Alpha Die Calculation) | **Neutral (중립)** : 기본 루틴 (순찰, 대기, 수면) |
| **금기 감지 연산** (Taboo Detection Calculation) | **Hostile (적대)** : 추적 (감정형 / 목적형 / 왜곡형) |
| **자정 임계치 연산** (Purification Threshold Calculation) | **Absolute Panic (자아 붕괴)** : 무장 해제 / 아군 오사 |

## 1.2. 내부 연산 정의

1. 비헤이비어 트리(FSM) 안에서도 내부 연산을 호출해서 실행될 수 있으며, 환경 트리거에 의해 독립적으로도 실행됩니다.

* **[우호도 연산 (Hostility)]**
* 몬스터의 인지 범위(Collider) 내에 플레이어가 진입했을 때 실행되는 연산이다.
* 플레이어의 스탯(왜곡도)을 기반으로 적대 수치를 연산하고 FSM 상태 전환 여부를 판단한다.


* **[알파 사망 연산 (Alpha Die)]**
* 피격 연산에서 룸(Room) 내의 리더 개체 HP가 0 이하가 되면 호출되는 연산이다.
* 해당 룸에 속한 하위 개체들의 상태를 강제로 [Intimidated(위압)] 상태로 전환하는 처리를 담당한다.


* **[자정 임계치 연산 (Purification)]**
* 특정 팩션의 맵 점유율(`Control_Ratio`)을 상시 검사하는 연산이다.
* 점유율이 95% 이상일 경우, 대규모 오프스크린(Off-screen) 엔티티 파괴 및 스폰 연산을 실행한다.
